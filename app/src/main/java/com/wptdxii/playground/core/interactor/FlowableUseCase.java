...
public abstract class FlowableUseCase<Request, Response> {
    ...
    public FlowableUseCase(@NonNull ThreadExecutor threadExecutor, @NonNull PostExecutionThread executionThread) {
    ...
    public void subscribe(Request request, DisposableSubscriber<Response> subscriber) {
        DisposableSubscriber disposable = buildUseCase(request).subscribeOn(Schedulers.from(mThreadExecutor)).observeOn(mExecutionThread.getScheduler()).subscribeWith(subscriber);
